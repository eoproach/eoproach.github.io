...
function getFishPoolByJudgement(j){
  ...
  if(j.result === 'perfect'){
    const premium = source.filter(f => f.grade === '희귀' || f.grade === '영웅' || f.grade === '전설');
    return premium.length ? premium : source;
  }
  if(j.result === 'success'){
    const normal = source.filter(f => f.grade === '일반' || f.grade === '희귀');
    return normal.length ? normal : source;
  }
  return [];
}
