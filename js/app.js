...
const SHOP_ITEMS={
bait:[{id:'bait-10',icon:'🪱',name:'일반 미끼 10개',desc:'낚시 10회 도전 가능',price:300,type:'bait',amount:10},{id:'bait-50',icon:'🪱',name:'일반 미끼 50개',desc:'꾸준한 낚시용 묶음 상품',price:1350,type:'bait',amount:50},{id:'bait-100',icon:'🪱',name:'일반 미끼 100개',desc:'장시간 수집용 대용량 상품',price:2500,type:'bait',amount:100}],
...
const RANKING_DATA = {
  weekly:[
    {name:'호수낚시러',icon:'🐟',level:18,point:12800,label:'주간 42마리'},
    {name:'월광펭귄',icon:'🌙',level:15,point:9400,label:'주간 31마리'},
    {name:'안개강태공',icon:'🌫️',level:12,point:7200,label:'주간 24마리'},
    {name:'산호수집가',icon:'🪸',level:10,point:5100,label:'주간 18마리'},
    {name:'초보낚시왕',icon:'🎣',level:8,point:3200,label:'주간 12마리'}
  ],
  total:[
    {name:'사이다낚시왕',icon:'🏆',level:88,point:1280000,label:'전체 4,820마리'},
    {name:'심해탐험가',icon:'🌊',level:76,point:842000,label:'전체 3,210마리'},
    {name:'전설수집가',icon:'✨',level:64,point:531000,label:'전체 2,040마리'},
    {name:'월광강태공',icon:'🌙',level:52,point:294000,label:'전체 1,130마리'},
    {name:'낚시연구원',icon:'📖',level:39,point:151000,label:'전체 620마리'}
  ],
  mine:[]
};
...
function getPlayerRankingRow(tab){
  const player = getPlayerLevelInfo();
  const weeklyPoint = Number(state.weeklyEarnedPoint || 0);
  const totalPoint = Number(state.totalEarnedPoint || 0);
  const weeklyCaught = Number(state.weeklyFishCaught || 0);
  const totalCaught = Number(state.totalFishCaught || 0);
  const point = tab === 'weekly' ? weeklyPoint : totalPoint;
  const caught = tab === 'weekly' ? weeklyCaught : totalCaught;
  return {
    rank:0,
    mine:true,
    name:'펭펭이',
    ...
    level:player.level,
    point,
    label:`${tab === 'weekly' ? '주간' : '전체'} ${caught.toLocaleString()}마리 · 도감 ${state.discoveredFishIds.size.toLocaleString()}종`
  };
}

function buildRankingRows(tab){
  if(tab === 'mine'){
    const totalRows = buildRankingRows('total');
    const mine = totalRows.find(row => row.mine) || getPlayerRankingRow('total');
    return [mine];
  }

  const base = (RANKING_DATA[tab] || []).map(item => ({...item, mine:false}));
  const mine = getPlayerRankingRow(tab);
  const rows = [...base, mine]
    .sort((a,b) => Number(b.point || 0) - Number(a.point || 0))
    .map((item, index) => ({...item, rank:index + 1}));

  const mineIndex = rows.findIndex(item => item.mine);
  if(mineIndex > 9){
    return [...rows.slice(0, 9), rows[mineIndex]];
  }
...
function renderRanking(){
  ...
  const rows = buildRankingRows(state.currentRankingTab);
  ...
  if(!rows.length){
    list.innerHTML = `<li class="empty">
      ...
      <span>🐧</span>
      <div>
        <strong>아직 내 순위 기록이 없습니다.</strong>
  ...
  list.innerHTML = rows.map(item => {
    ...
    return `<li class="${item.mine ? 'my' : ''}">
      ...
      <em>${Number(item.point || 0).toLocaleString()}P</em>
    </li>`;
  }).join('');
}
...
function applyFishingData(j, fish, reward){
  ...
  state.totalEarnedPoint = Number(state.totalEarnedPoint || 0) + Number(reward.point || 0);
  state.weeklyEarnedPoint = Number(state.weeklyEarnedPoint || 0) + Number(reward.point || 0);
