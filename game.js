...
const rarities = [
  {key:'common', name:'일반', rate:60, gold:10, exp:5, emoji:'🐟', stars:1},
  {key:'uncommon', name:'고급', rate:25, gold:30, exp:15, emoji:'🐠', stars:2},
  {key:'rare', name:'희귀', rate:10, gold:100, exp:50, emoji:'🐡', stars:3},
  {key:'epic', name:'영웅', rate:4, gold:500, exp:200, emoji:'🦈', stars:4},
  {key:'legendary', name:'전설', rate:1, gold:3000, exp:1000, emoji:'🐉', stars:5}
];
...
function showCatchModal(picked, value, first){
  ...
  $('catchIcon').textContent = picked.rarity.emoji;
...
function renderCollection(){
  $('collectionGrid').innerHTML = maps.flatMap((m,mi)=>m.fish.map((fish,fi)=>({m,mi,fish,fi}))).map(({mi,fish,fi})=>{
    ...
    return `<article class="fish-card ${known?'':'unknown'}">
      ...
      <div class="fish-icon">${known ? r.emoji : '❔'}</div>
...
function renderShop(){
  ...
  $('shopGrid').innerHTML = `
    <article class="shop-card"><div class="product">🐛</div><strong>미끼 x10</strong><p>가격: 100 골드</p><button type="button" onclick="buyGoldBait()">🪙 100</button></article>
    <article class="shop-card"><div class="product">🪱</div><strong>고급 미끼 x30</strong><p>가격: 다이아 10개</p><button type="button" onclick="buyBait(30,10)">💎 10</button></article>
    <article class="shop-card"><div class="product">🌟</div><strong>전설 미끼 x1</strong><p>전설 확률 보너스 예정</p><button type="button" onclick="buyBait(1,50)">💎 50</button></article>
    <article class="shop-card"><div class="product">🎣</div><strong>${rodGrade} +${state.rodLevel}</strong><p>강화 비용: ${upgradeCost.toLocaleString()} 골드</p><button type="button" onclick="upgradeRod()">강화하기</button></article>`;
}
